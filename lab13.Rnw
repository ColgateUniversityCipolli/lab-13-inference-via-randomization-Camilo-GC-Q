...
for the closer, further, and difference data, so perform the randomization procedure
  ...
  CI_randomization = function(data, alpha = 0.05, step = 0.0005){
    mu_hat = mean(data)
    mu0_low = mu_hat
    while(pval_randomization(data, mu0_low) < alpha){
      mu0_low = mu0_low - step
    }
    
    mu0_high = mu_hat
    while(pval_randomization(data, mu0_high) < alpha){
      mu0_high = mu0_high + step
    }
  ...
  finch_data = read_csv("zebrafinches.csv")
  CI_randomization(finch_data$further)
  CI_randomization(finch_data$closer)
  CI_randomization(finch_data$diff)
